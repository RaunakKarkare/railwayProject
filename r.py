...
def create_action_decision_flowchart():
    """
    Create a flowchart visualizing the action decision process in the DQN model.
    Returns:
    ...
    # Define nodes (rectangles and circles)
    nodes = [
        {
        ...
        {"text": "DQN Policy\n(Select Action via Q-Values)", "xy": (5, 8), "shape": "rect", "size": (3, 1)},
    ...
    # Draw arrows
    arrows = [
        ((5, 12.25), (5, 11.25)),  # State to Adjustments
        ((5, 9.75), (5, 8.75)),  # Adjustments to DQN
        ((5, 7.25), (5, 6.75)),  # DQN to Action
        ((5, 5.25), (5, 4.75)),  # Action to Reward
        ((5, 3.25), (5, 2.75)),  # Reward to Update
        ((5, 1.75), (5, 1))  # Update to Next
    ]
    for (x1, y1), (x2, y2) in arrows:
        ax.add_patch(FancyArrowPatch((x1, y1), (x2, y2), mutation_scale=20, color='black'))

    plt.title("Action Decision Process in DQN Model", fontsize=12, pad=20)
    return fig
...
if df_optimized.empty:
    ...
    st.write("This flowchart illustrates how parameters influence action selection in the DQN model.")
