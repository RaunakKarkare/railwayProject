...
def load_data_train_and_predict(_hyperparams, api_key, train_numbers, date, debug=False):
    ...
    if debug:
        ...
        st.write(f"MPS built: {torch.backends.mps.is_built()}")
        ...
        for lr in learning_rates:
            try:
                model = algo_class(
                    ...
                    learning_rate=lr,
                    verbose=0,
                    device=device  # Explicitly use MPS or CPU
                )
        ...
        with torch.no_grad():
            for train_no, group in train_groups.items():
                ...
                for i in range(len(group)):
                    ...
                    state_tensor = torch.as_tensor(state.reshape(1, -1), dtype=torch.float32).to(device)
                    if continuous:
