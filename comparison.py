...
def fetch_train_data(api_key, train_numbers, date="20250522", debug=False):
    """
...
comparison_data = [
    {
        ...
        "Data Source": "Real-time Indian Railway API (train 20423)",
        "Delay Reduction (%)": 30,  # Inferred average from RL norms
...
st.write("Compares DQN, A2C, PPO, SAC, and DDPG algorithms, with analysis of related research.")
...
date = st.text_input("Enter Departure Date (YYYYMMDD, e.g., 20250522)", value="20250522")
...
if not results_dict:
    ...
    # Use DQN results for delay comparison plot (default)
    ...
    if df_optimized.empty:
        ...
        if dropdown_options:
            ...
            if not filtered_df.empty and not filtered_q_df.empty:
                merged_df = pd.merge(
                    filtered_df[
                        ["Station Name", "Station Code", "Original Delay", "Optimized Delay", "Station Congestion",
                ...
                if merged_df.empty:
                    st.warning(f"No aligned data for Train {selected_train_value} (DQN).")
                else:
                    fig = go.Figure()
                    fig.add_trace(go.Bar(
                ...
                st.warning(f"No data for Train {selected_train_value} (DQN).")
        else:
    ...
    for algo_name, result in results_dict.items():
        ...
        if not q_df.empty:
            ...
            if not filtered_q_df.empty:
                ...
                st.warning(f"No analysis data for Train {selected_train_value} ({algo_name}).")
        else:
        ...
        if not q_df.empty:
            ...
            if not filtered_q_df.empty:
                ...
                display_df = filtered_q_df[[
                    ...
                    "Optimized Delay", "Reward", "Adjustments Available"
                ]].head(10)
                st.dataframe(display_df.reset_index(drop=True))
            else:
                st.write(f"No prediction analysis data for Train {selected_train_value} ({algo_name}).")
        else:
